...
@Component({
  selector: 'app-save-w-detail',
  templateUrl: './save-w-detail.component.html',
  styleUrls: ['./save-w-detail.component.scss']
})
export class SaveWDetailComponent implements OnInit {
  maintenanceDate: string;
  waterSupply = ['Kiểm tra bể chứa nước', 'Bơm nước', 'Kiểm tra đường ống cấp nước', 'Vệ sinh bồn chứa nước'];
  drainage = ['Kiểm tra đường ống thoát nước', 'Kiểm tra hoạt động của thiết bị vệ sinh'];
  description = 'Kiểm tra bể chứa nước';
  waterType = [];
  ...
  devis = [];
  constructor(
    public dialogRef: MatDialogRef<SaveWDetailComponent>,
  ...
  ngOnInit() {
    this.waterType = this.waterSupply;
  ...
  changeWaterType(value) {
    ...
    if (value === '0') {
      ...
      this.waterType = this.drainage;
    }
    this.description = this.waterType[0];
  }
}
