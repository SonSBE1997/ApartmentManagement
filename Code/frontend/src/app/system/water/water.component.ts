...
import { SaveWDetailComponent } from './save-w-detail/save-w-detail.component';
@Component({
  selector: 'app-water',
...
export class WaterComponent implements OnInit {
  ...
  loadData() {
    this.maintenances = [];
    this.maintenanceService.findByDeviceGroup(4).subscribe(m => {
      m.forEach(v => {
        v.persons.forEach(p => {
          if (p.supervisor) {
  ...
  showDetail(row) {
    const dialogRef = this.dialog.open(WDetailComponent, {
      width: '600px',
      data: row,
      position: { top: '50px' }
    });
  ...
  save() {
    const dialogRef = this.dialog.open(SaveWDetailComponent, {
      width: '700px',
      data: '',
      position: { top: '50px' },
      disableClose: true,
