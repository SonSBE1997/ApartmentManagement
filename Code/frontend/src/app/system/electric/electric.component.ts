...
import { SaveEDetailComponent } from './save-e-detail/save-e-detail.component';
...
export class ElectricComponent implements OnInit {
  ...
  loadData() {
    this.maintenances = [];
    this.maintenanceService.findByDeviceGroup(2).subscribe(m => {
      m.forEach(v => {
        v.persons.forEach(p => {
          if (p.supervisor) {
  ...
  showDetail(row) {
    const dialogRef = this.dialog.open(EDetailComponent, {
      width: '600px',
      data: row,
      position: { top: '50px' }
    });
  ...
  save() {
    const dialogRef = this.dialog.open(SaveEDetailComponent, {
      width: '700px',
      data: '',
      position: { top: '50px' },
      disableClose: true,
