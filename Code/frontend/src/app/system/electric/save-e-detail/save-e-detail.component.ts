...
@Component({
  selector: 'app-save-e-detail',
  templateUrl: './save-e-detail.component.html',
  styleUrls: ['./save-e-detail.component.scss']
})
export class SaveEDetailComponent implements OnInit {
  maintenanceDate: string;
  description = 'Kiểm tra thiết bị điện công cộng';
  ...
  devis = [];
  constructor(
    public dialogRef: MatDialogRef<SaveEDetailComponent>,
  ...
  ngOnInit() {
    ...
    this.deviceService.findAllByDeviceGroup(2).subscribe(v => {
      v.forEach(d => {
        this.devices.push(d);
      });
    });
