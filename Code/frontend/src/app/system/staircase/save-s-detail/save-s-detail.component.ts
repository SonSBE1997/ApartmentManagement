...
@Component({
  selector: 'app-save-s-detail',
  templateUrl: './save-s-detail.component.html',
  styleUrls: ['./save-s-detail.component.scss']
})
export class SaveSDetailComponent implements OnInit {
  maintenanceDate: string;
  description = 'Kiểm tra vận hành thang máy';
  ...
  devis = [];
  constructor(
    public dialogRef: MatDialogRef<SaveSDetailComponent>,
  ...
  addDevice() {
    this.devis.push({ id: 0, description: '', location: '' });
  }

  selectDeviceChange(event, index) {
    const value = parseInt(event.target.value, 10);
    if (value === 0) {
      this.devis[index].id = 0;
      this.devis[index].description = '';
      this.devis[index].location = '';
    } else {
      const d = this.devices.find(v => v.id === value);
      const inArr = this.devis.find(v => v.id === value);
      if (inArr === null || inArr === undefined) {
        this.devis[index].id = d.id;
      } else {
        this.notifierService.notify(
          'warning',
          'Loại thang máy đã được chọn này đã được chọn'
        );
        this.devis[index].id = 0;
        this.devis[index].description = '';
        this.devis[index].location = '';
  ...
  save() {
    ...
    this.devis = this.devis.filter(v => v.id !== 0);
    ...
    for (let i = 0; i < detailLength; i++) {
      const d = this.devices.find(v => v.id === this.devis[i].id);
      this.details.push({
        id: 0,
        price: 0,
