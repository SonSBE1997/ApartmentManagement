...
import { FDetailComponent } from './f-detail/f-detail.component';
import { SaveFDetailComponent } from './save-f-detail/save-f-detail.component';
...
export class FireComponent implements OnInit {
  ...
  loadData() {
    this.maintenances = [];
    this.maintenanceService.findByDeviceGroup(3).subscribe(m => {
      m.forEach(v => {
        v.persons.forEach(p => {
          if (p.supervisor) {
  ...
  showDetail(row) {
    const dialogRef = this.dialog.open(FDetailComponent, {
      width: '600px',
      data: row,
      position: { top: '50px' }
    });
  ...
  save() {
    const dialogRef = this.dialog.open(SaveFDetailComponent, {
      width: '700px',
      data: '',
      position: { top: '50px' },
      disableClose: true,
