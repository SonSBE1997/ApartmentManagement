...
import { SaveRepairComponent } from './save-repair/save-repair.component';
import { ADetailComponent } from './a-detail/a-detail.component';

@Component({
  selector: 'app-repair-apt',
  templateUrl: './repair-apt.component.html',
  styleUrls: ['./repair-apt.component.scss']
})
export class RepairAptComponent implements OnInit {
  ...
  loadData() {
    this.maintenances = [];
    this.maintenanceService.findByDeviceGroup(5).subscribe(m => {
      m.forEach(v => {
        v.persons.forEach(p => {
          if (p.supervisor) {
  ...
  filter() {
    ...
    this.maintenances = this.maintenances.filter(v => {
      const time = new Date(v.maintenanceDate).getTime();
      return time >= comeDate.getTime() && time <= leaveDate.getTime();
    });
  ...
  showDetail(row) {
    const dialogRef = this.dialog.open(ADetailComponent, {
      width: '600px',
      data: row,
      position: { top: '50px' }
    });
  ...
  save() {
    const dialogRef = this.dialog.open(SaveRepairComponent, {
      width: '700px',
      data: '',
      position: { top: '50px' },
      disableClose: true,
